...
-module(canillita_emergencies_sse_handler).
...
-spec notify(Event::canillita_emergencies:emergency()) -> ok.
notify(Event) ->
  lists:foreach(
    fun(Listener) ->
...
init(_InitArgs, LastEventId, Req) ->
  ...
  NewsItems = canillita_emergencies_repo:fetch_since(LastEventId),
  News = [canillita_emergencies:to_sse(NewsItem) || NewsItem <- NewsItems],
...
-spec handle_notify( NewsItem::canillita_emergencies:emergency()
                   , State::state()
                   ) -> {send, lasse_handler:event(), state()}.
handle_notify(NewsItem, State) ->
  Event = canillita_emergencies:to_sse(NewsItem),
  {send, Event, State}.
